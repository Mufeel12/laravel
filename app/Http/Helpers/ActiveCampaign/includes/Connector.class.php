...
class AC_Connector {
	...
	public function curl($url, $params_data = array(), $verb = "", $custom_method = "") {
		...
		if ($this->debug) {
			$this->dbg($url, 1, "pre", "Note: Request URL");
		}
		if ($verb == "POST" || $verb == "PUT" || $verb == "DELETE") {
			...
			if ($this->debug) {
				...
				$this->dbg($data, 1, "pre", "Note: POST data");
			}
		...
		if ($this->debug) {
			$this->dbg($response, 1, "pre", "Note: Raw response");
		}
		...
		if ($this->debug) {
			$this->dbg($http_code, 1, "pre", "Note: Response HTTP code");
			$request_headers = curl_getinfo($request, CURLINFO_HEADER_OUT);
			$debug_str1 .= "\$request_headers = curl_getinfo(\$ch, CURLINFO_HEADER_OUT);\n";
			$this->dbg($request_headers, 1, "pre", "Note: Request headers");
		}
		curl_close($request);
		$debug_str1 .= "curl_close(\$ch);\n";
		$object = json_decode($response);
		if ($this->debug) {
			$this->dbg($object, 1, "pre", "Note: Response object (json_decode)");
		}
